	...
	var pluginName = 'horizontalTimeline',
		defaults = {
			...
			iconClass: {
				...
				"prev": "fa fa-angle-left fa-3x timeline-btn",
				"next": "fa fa-angle-right fa-3x timeline-btn",
...
_updateTimelinePosition: function (event, timelineComponents, timelineTotalWidth) {
  function dotCenter($a){
    var aLeft = parseFloat($a.css('left')) || 0;
    var el = $a[0];
    var cs = window.getComputedStyle(el, '::after');

    var leftStr = cs.left || '0px';
    var wStr    = cs.width || '0px';

    function toPx(str, ref){
      str = String(str);
      return str.endsWith('%') ? (parseFloat(str)/100) * ref : parseFloat(str) || 0;
    }

    var dotLeft = toPx(leftStr, $a.width());
    var dotW    = toPx(wStr,   $a.width());
    return aLeft + dotLeft + dotW/2;
  }

  var center = dotCenter(event);
  var timelineWidth = timelineComponents['timelineWrapper'].width();

  // Allow extra left scroll so the last dot can reach center
  var endBuffer    = timelineWidth / 2;
  var minTranslate = (timelineWidth - timelineTotalWidth) - endBuffer;

  var desired = -center + (timelineWidth/2);
  if (desired > 0) desired = 0;
  if (desired < minTranslate) desired = minTranslate;

  this._translateTimeline(timelineComponents, desired, minTranslate);
},
...
_setTimelineWidth: function (timelineComponents) {
  // Helper: get the CENTER (in px) of a dot drawn by the anchor's ::after
  function dotCenter($a){
    var aLeft = parseFloat($a.css('left')) || 0;     // anchor left within .events
    var el = $a[0];
    var cs = window.getComputedStyle(el, '::after'); // pseudo styles

    // ::after left can be %, convert relative to anchor width
    function toPx(str, ref){
      str = String(str || '0');
      return str.endsWith('%') ? (parseFloat(str)/100) * ref : parseFloat(str) || 0;
    }
    ...
    var aW      = $a.width();
    var dotLeft = toPx(cs.left,  aW);                // e.g., "40%" -> px
    var dotW    = toPx(cs.width, aW);                // e.g., "24px"
    return aLeft + dotLeft + dotW/2;                 // center of the circle
  }

  var totalWidth = 0;
  var wrapperWidth = timelineComponents['timelineWrapper'].width();
  var $events = timelineComponents['timelineEvents'];

  // If we somehow only have one event, keep it simple
  if ($events.length <= 1) {
    totalWidth = wrapperWidth;
    timelineComponents['eventsWrapper'].css({
      width: totalWidth + 'px',
      background: '' // no custom paint
    });
  ...
  // Centers of first and last dots
  var $first = $events.first();
  var $last  = $events.last();
  var start  = dotCenter($first);
  var end    = dotCenter($last);

  // Add right-side slack so the last dot can be centered in view
  var endBuffer   = wrapperWidth / 2;                // tweak if you want more slack
  var neededWidth = Math.max(wrapperWidth, end + endBuffer);

  totalWidth = neededWidth;

  // Set the overall width and keep normal positioning logic working
  timelineComponents['eventsWrapper'].css('width', totalWidth + 'px');

  // Repaint the track only between the two dot centers
  // +1px ensures the line visually "touches" the last dot at some zoom levels
  var paintTo = end + 1;
  var bg = 'linear-gradient(to right,' +
           ...
           '#000 ' + start + 'px, ' +
           '#000 ' + paintTo + 'px, ' +
           'transparent ' + paintTo + 'px, ' +
           'transparent 100%)';
  timelineComponents['eventsWrapper'].css('background', bg);

  // Keep existing behavior that recenters based on selected item
  this._updateTimelinePosition(
    timelineComponents['eventsWrapper'].find('a.selected'),
    timelineComponents,
    totalWidth
